...
/**
 * @brief Print the short usage message for the program.
 * 
...
/**
 * @brief Print the full help message for command line arguments.
 * 
 * @param progname The name of the program (usually argv[0]).
 */
...
/**
 * @brief Parse command-line arguments and store parsed options.
 * 
 * @param argc Argument count from main.
 * @param argv Argument vector from main.
 * @param cycles Number of simulation cycles.
 * @param tracefile Pointer to trace file path.
 * @param input_file Pointer to input request file path.
 * @param endianness Endianness flag (0 = little, 1 = big).
 * @param latency_scrambling Latency in clock cycles for scrambling.
 * @param latency_encrypt Latency for encryption/decryption.
 * @param latency_memory_access Latency for memory access.
...
static uint32_t parse_uint32(const char *inputString)
{
    ...
    if (*endptr != '\0' || errno == ERANGE || val > UINT32_MAX)
        return -1;
    return (uint32_t)val;
}
...
static uint8_t parse_uint8(const char *inputString)
{
    ...
    uint64_t val;
    val = strtoull(inputString, &endptr, 10);
    if (*endptr != '\0' || errno == ERANGE || val > UINT8_MAX)
        return -1;
    return (uint8_t)val;
}
...
/**
 * @brief Parse the next CSV line and extract the request fields. When facing an empty line, it will jump to the next line with content.
 * 
...
int parse_requests(const char *input, uint32_t *numRequests, struct Request **requests)
{
    ...
    uint32_t count = 0;
    uint32_t capacity = 8192; // our default capacity for requests
...
/**
 * @brief Validate if the file path is a regular readable file and open it.
 * 
 * @param path File path string.
 * @return FILE* Valid file pointer on success, NULL on failure.
 */
...
/**
 * @brief Main function: parses arguments, loads requests, and runs simulation.
 * 
