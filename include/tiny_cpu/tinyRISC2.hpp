...
SC_MODULE(TINY_RISC) {
  ...
  sc_signal<sc_bv<4>> faultbit_sig;
  sc_signal<uint32_t> fault_sig;
  ...
  TINY_RISC(sc_module_name name, std::map<uint32_t, uint32_t> initialMemory, uint32_t period) : 
      ...
      pc("pc"), rf("rf"), alu("alu"), smu("smu",0,0,0,0,0), cu("cu"),
    ...
    smu.wdata.bind(muxReg2_wdata_mm);
    ...
    // 1.4
    smu.rdata.bind(mm_rdata_muxMemR);
    ...
    smu.addr.bind(muxMemAddr_addr_mm);
